    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden pt-4">
      ...
      >
        ...
        <div className="-mx-1 min-h-0 flex-1 overflow-y-auto px-1 pb-8 pt-1">
          <TabsContent value="servers" className="space-y-7">
            <ServerConnectionSetting />
            <ServerSettings />
          </TabsContent>
          <TabsContent value="general" className="space-y-7">
            ...
            <SettingsSection title="分组与安装" icon={<Package />}>
              <SettingCard
                ...
                description="开启后保留手动安装的 skill；关闭后仅当前启用分组中的 skill 生效，组外 skill 暂时停用，文件保留。没有启用分组时同样生效。重新开启只恢复由此策略停用的 skill；退出应用管理时恢复原状态。此策略仅适用于已接入原生启停的 Codex 和 Claude Code；OpenCode、Pi、Grok Build 的手动 Skill 始终保留。"
                icon={<BookmarkCheck />}
              >
                <Switch
                  id="preserve-manual"
