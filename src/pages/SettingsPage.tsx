...
export function SettingsPage() {
  ...
  return (
    <div className="min-h-0 flex-1 space-y-6 overflow-y-auto py-4 pb-8">
      ...
      <section className="space-y-2">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          ...
          <code>CODEX_HOME</code>） 或标准位置解析。如果这些变量只写在 shell
          配置里、图形界面进程读不到， 可以在这里显式指定 ——
          这里的设置优先级最高。
        </p>
        <ListContainer>
          {agents.map((a, i) => {
      ...
      <section className="space-y-2">
        ...
        <ListContainer>
          ...
          <ListItemRow isLast>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium">配置目录</p>
              <p className="truncate font-mono text-[11px] text-muted-foreground">
                {configDir}
              </p>
            </div>
      ...
      <section className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          Skill Studio {version ? `v${version}` : ""} · 跨 agent 的 Agent Skill
          管理器
        </p>
        <p className="text-xs leading-relaxed text-muted-foreground">
          停用 skill 走各 agent 自己的配置开关（Claude 的{" "}
          <code>skillOverrides</code>、Codex 的 <code>[[skills.config]]</code>
          ）， 不删文件，随时可恢复。
        </p>
      </section>
