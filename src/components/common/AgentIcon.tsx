...
/**
 * OpenAI 品牌标记，Codex 用。同样取自 cc-switch 的 `openai.svg`。
 * 这个标记是单色的，用 currentColor 跟随文字色 —— cc-switch 的图标元数据里
 * openai 的 defaultColor 也正是 currentColor，深色模式下自动翻成白色，
 * 比 `brightness-0 invert` 那套滤镜干净。
 */
...
/**
 * 按 agent 取品牌标记的组件形态，给只接受组件类型的插槽用
 * （例如 `EmptyState` 的 icon）。返回的是 map 里的稳定引用，
 * 不会每次渲染都造一个新组件类型导致重挂载。
 */
export function agentIcon(agentId: string): ComponentType<MarkProps> {
